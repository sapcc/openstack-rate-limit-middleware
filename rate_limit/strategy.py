...
class RateLimitStrategy(object):
    ...
    """
    def __init__(self, memcache_host, rate_limit_response, clock_accuracy,
                 logger=logging.getLogger(__name__), strategy_name='Default', **kwargs):
        """
        initializes a rate limit strategy

        :param memcached: the memcached connection
        :param key_function: function that generates a key
        :param rate_limit_response: the configurable rate limit response
        :param clock_accuracy: the accuracy of the clocks
        ...
        if kwargs and kwargs.get('memcached'):
            self.memcached = kwargs.get('memcached')
        else:
            self.memcached = self._new_memcache(memcache_host)
    ...
    def key_func(self, scope, action, target_type_uri):
        ...
        """
        return common.key_func(scope, action, target_type_uri)

    def _new_memcache(self, memcached_host):
        """
        initializes a new memcache client

        :param memcached_host: memcache address
...
class FixedWindowStrategy(RateLimitStrategy):
    ...
    """
    def __init__(self, memcache_host, rate_limit_response, clock_accuracy,
                 logger=logging.getLogger(__name__), strategy_name='FixedWindow', **kwargs):
        ...
        self.max_sleep_time_seconds = kwargs.get('max_sleep_time_seconds', 20)
        self.rate_buffer_seconds = kwargs.get('rate_buffer_seconds', 20)
        self.clock_accuracy = clock_accuracy
        self.rate_limit_response = rate_limit_response

    def rate_limit(self, scope, action, target_type_uri, max_rate_string):
        key = self.key_func(scope, action, target_type_uri)
        try:
...
class SlidingWindowStrategy(RateLimitStrategy):
    ...
    """
    def __init__(self, memcache_host, rate_limit_response, clock_accuracy,
                 logger=logging.getLogger(__name__), strategy_name='SlidingWindow', **kwargs):
        super(SlidingWindowStrategy, self).__init__(
            logger=logger, memcache_host=memcache_host,
            clock_accuracy=clock_accuracy,
            rate_limit_response=rate_limit_response,
            strategy_name=strategy_name,
            kwargs=kwargs
    ...
    def key_with_time_window(self, scope, action, target_type_uri, time_window):
        ...
        """
        return '{0}_{1}'.format(self.key_func(scope, action, target_type_uri), time_window)
