...
class TestOpenStackRateLimitMiddleware(unittest.TestCase):
    ...
    def test_get_rate_limit_and_strategy(self):
        ...
        for stim in stimuli:
            ...
            rate_limit, rate_strat = self.app._get_rate_limit_and_strategy(action, target_type_uri)
            self.assertEqual(
                rate_limit,
                expected_ratelimit,
                "rate limit for '{0} {1}' should be '{2}'".format(action, target_type_uri, expected_ratelimit)
            )
            self.assertEqual(
                rate_strat,
                expected_strategy,
                "rate limit strategy for '{0} {1}' should be '{2}'".format(action, target_type_uri, expected_strategy)
            )
