...
class TestResponse(unittest.TestCase):
    def test_default_ratelimitexceededresponse_json(self):
        ...
        self.assertEqual(
            ...
            "expected response content type to be equal. want '{0}' but got '{1}'".format(
                common.Constants.content_type_json, ratelimit_response.content_type)
        )
    ...
    def test_custom_ratelimitexceededresponse_html(self):
        ...
        status, status_code, headers, body, json_body = response.response_parameters_from_config(
            conf.get(common.Constants.ratelimit_response))
    ...
    def test_default_blacklistresponse(self):
        ...
        expected_json_body = json.dumps(
            {"error": {"status": "497 Blacklisted", "message": "You have been blacklisted"}}, sort_keys=True)
        self.assertEqual(
            blacklist_response.json_body,
            expected_json_body,
            "expected blacklist response body to be '{0}' but got '{1}'".format(expected_json_body,
                                                                                blacklist_response.json_body)
        )
    ...
    def test_custom_blacklistresponse_json(self):
        conf = common.load_config(SWIFTCONFIGPATH)
        status, status_code, headers, body, json_body = response.response_parameters_from_config(
            conf.get(common.Constants.blacklist_response))
        ...
        expected_json_body = json.dumps(
            {"error": {"status": "497 Blacklisted",
                       "message": "You have been blacklisted. Please contact and administrator."}},
            sort_keys=True)
